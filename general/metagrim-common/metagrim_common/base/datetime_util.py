...
def format_timespan_digits(ts):
    ...
    if ts.days:
        ...
        return f"{ts.days} {day_or_days}, " f"{ts.hours:02d}:{ts.minutes:02d}:{ts.seconds:02d}"
    if ts.seconds:
...
def format_timespan_str(ts):
    ...
    if ts.days:
        day_or_days = "days" if ts.days > 1 else "day"
        return f"{ts.days} {day_or_days} " f"{ts.hours:.0f} hours {ts.minutes:.0f} minutes {ts.seconds} seconds"
    if ts.hours:
